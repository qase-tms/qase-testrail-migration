...
class Logger:
    ...
    Levels are additive: each one includes everything above it in this list.
    ...
    migration that quietly drops custom fields must not look successful in
    the terminal. Everything else goes to the console only at 'verbose' or
    above, so the default run stays readable.
    """
    ...
    def __init__(self, level: str = 'info', write_to_file: bool = True, log_dir: str = './logs', prefix: str = ''):
        ...
        if self.write_to_file:
            ...
            filename = f'{prefix}_testrail_{timestamp}.log' if prefix else f'testrail_{timestamp}.log'
            ...
            with open(self.log_file, 'w', encoding='utf-8') as f:
                f.write(f'# qase-testrail-migration '
    ...
    def log(self, message: str, level: str = 'info'):
        ...
        # Too detailed for the configured level, drop it entirely
        ...
        # Failures and warnings are always shown. Everything else only when
        # the level is verbose or above.
        if severity <= self.LEVELS['warn']:
            color = self._COLORS.get(name, '0')
            # A leading newline so the message does not land on top of a
            # progress line, which print_status redraws with a carriage return
            print(f"\n\033[{color}m{line}\033[0m", file=sys.stderr, flush=True)
